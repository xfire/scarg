/**
 * generate index.html files for each directory for easier browsing.
 *
 * orginal can be found here: http://mtkopone.github.com/scct/
 * thanks mikko :)
 */
...
def generateIndex(dir: File) = {
  ...
  <html>
  <head><title>{ path(dir) }</title></head>
  <body>
    <h1>{ path(dir) }</h1>
    <ul>
      <li><a href="../">..</a></li>
...
def path(dir: File): String = if (dir == root) "scarg/maven-repo" else path(dir.getParentFile) + "/" + dir.getName
